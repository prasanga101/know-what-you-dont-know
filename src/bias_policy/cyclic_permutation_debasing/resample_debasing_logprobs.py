...
INPUT_FILE = "data/infra_check/debasic_ollama_log.json"
OUTPUT_FILE = "data/infra_check/debasic_ollama_logprob_resampling_result.json"
...
for item in sample:
    ...
    # NOTE: compare against the ROTATED correct letter, not the original —
    # the whole point of this run is that the correct answer moved.
    correct_letter = item["rotated_correct_letter"]
    correct_option_logprob = abcd_logprobs.get(correct_letter)
    correct_option_prob = abcd_probs.get(correct_letter)

    # Sanity invariant: at temperature=0 with penalties neutralized, the
    # model's own emitted token is by definition the argmax over the FULL
    # vocabulary, so if it emitted one of A/B/C/D, that letter MUST also be
    # the argmax within our extracted A/B/C/D subset. If it isn't,
    ...
    if has_logprobs and response_letter in TARGET_OPTIONS and top_option != response_letter:
        warning = (
            f"item={item['item_id']} shift={item['shift']}: top_option={top_option} "
    ...
    result = {
        ...
        "shift": item["shift"],
        "original_correct_letter": item["original_correct_letter"],
        "rotated_correct_letter": correct_letter,
        "rotated_options": item["rotated_options"],
    ...
    print(f"item={item['item_id']} shift={item['shift']} response={data.get('response')}")
    print(
        f"A={abcd_logprobs['A']} "
        f"B={abcd_logprobs['B']} "
        f"C={abcd_logprobs['C']} "
    ...
    print(f"top_option={top_option} rotated_correct={correct_letter} ok={top_option == correct_letter}")
    print(f"has_logprobs={has_logprobs} error={error}")
    print("-" * 80)

assert len(results) == 48, f"expected 48 results, got {len(results)}"
...
print(f"Saved {len(results)} logprob resampling results to {OUTPUT_FILE}")
...
if warnings:
    ...
    print("\nNo extraction warnings — top_option matches the model's own emitted letter on every record.")